...
import {useAreaDataContext, useAreaSetContext} from "../../context/AreaContext";
...
export default function Header(props) {
  ...
  var panelSettings = usePanelState();
  var panelName = panelSettings.name;
  var HeaderComment = panelSettings.HeaderComment;
    ...
    <AppBar position="fixed" className={classes.appBar}>
      <Toolbar className={classes.toolbar}>
        ...
        >
          ...
          >
            <BusinessIcon className={classes.profileMenuIcon} /> {area.description}
          </MenuItem>
          <MenuItem
            className={classNames(
              classes.profileMenuItem,
...
function SelectArea({classes}){
  ...
  const [{areas,roles},setAreasRoles] = useState({});
  ...
  useEffect(()=>{
    (async ()=> {
      ...
      const areas = Object.keys(pUser.areas||{}).map(r=>pUser.areas[r]);
      const roles = Object.keys(pUser.roles||{}).map(r=>pUser.roles[r]);
      setAreasRoles({areas,roles});
    })()
  },[getuser.areas]);


  return areas ? <div className={classes.formControl}>
  <FormControl>
    ...
    >
      {areas.map(a=><option value={a.objectID} key={a.objectID}>{a.name}</option>)}
    </Select>
  </FormControl>
  </div> : <></>
}
